...
size_t D12GraphicsPlannerModule::GetSplitExecutionSize() { return Math::SplitJobs(GetExecutionSize(), 4, 10); }
...
void D12GraphicsPlannerModule::RecSetRenderPass(const D12RenderPass* target, bool ignoreLoadAction)
{
	auto buffer = ContinueRecording();
	auto& stream = buffer->stream;
...
void D12GraphicsPlannerModule::RecDraw(const DrawDesc& target)
{
	...
	for (size_t i = 0; i < rootParameters.size(); i++)
	{
		...
		{
		case D12RootParamterType::TableSRV:
		case D12RootParamterType::TableSamplers:
		{
			stream.Write(rootArgument.memory);
			break;
		}
		case D12RootParamterType::ConstantBuffer:
		{
...
void D12GraphicsPlannerModule::RecSetHeap(const D12DescriptorHeap** heap)
{
	...
	stream.Write(heap, sizeof(D12DescriptorHeap*) * Enum::ToUnderlying(D12HeapType::Count));
	stream.Align();
	buffer->commandCount++;
}
...
bool D12GraphicsPlannerModule::ExecuteCommand(const ExecutionContext& context, D12CmdBuffer* buffer, CommandCode commandCode)
{
	...
	switch (commandCode)
	{
		...
		DESERIALIZE_METHOD_ARG2_START(SetRenderPass, D12RenderPass, target, bool, ignoreLoadAction);
		...
		ID3D12DescriptorHeap* heaps[] = {
			buffer->heaps[Enum::ToUnderlying(D12HeapType::SRVs)]->Get_heap(),
			buffer->heaps[Enum::ToUnderlying(D12HeapType::Samplers)]->Get_heap() };
		...
		for (size_t i = 0; i < rootParameters.size(); i++)
		{
			...
			case D12RootParamterType::TableSRV:
			{
				ASSERT(buffer->heaps[Enum::ToUnderlying(D12HeapType::SRVs)] != nullptr);
				auto& value = stream.FastRead<D12HeapMemory>();
				auto handle = buffer->heaps[Enum::ToUnderlying(D12HeapType::SRVs)]->GetGpuHandle(value);
			...
			case D12RootParamterType::TableSamplers:
			{
				ASSERT(buffer->heaps[Enum::ToUnderlying(D12HeapType::Samplers)] != nullptr);
				auto& value = stream.FastRead<D12HeapMemory>();
				auto handle = buffer->heaps[Enum::ToUnderlying(D12HeapType::Samplers)]->GetGpuHandle(value);
		...
		stream.Read(buffer->heaps, sizeof(D12DescriptorHeap*) * Enum::ToUnderlying(D12HeapType::Count));
		DESERIALIZE_METHOD_END;
	}
	return false;
}