...
public:
	...
	void RecSetRenderPass(const D12RenderPass* target, bool ignoreLoadAction = false);
...
private:
	...
	struct RecingOptimizer
	{
		...
		inline void MarSetHeap(D12DescriptorHeap** heaps) { memcpy((void*)lastHeaps, (void*)heaps, sizeof(D12DescriptorHeap*) * Enum::ToUnderlying(D12HeapType::Count)); }
		inline bool ShouldSplitRecording() { return drawCount == 500; }
		
		D12RenderPass* lastRenderPass;
		D12DescriptorHeap* lastHeaps[Enum::ToUnderlying(D12HeapType::Count)];
		size_t drawCount;
	};
