...
void D12GraphicsModule::SetRenderPass(const ExecutionContext & context, const D12RenderPass * target)
{
	...
	D12DescriptorHeap* heaps[Enum::ToUnderlying(D12HeapType::Count)] = {
		srvHeap,
		samplersHeap,
		nullptr,
		nullptr,
...
bool D12GraphicsModule::Initialize()
{
	...
	srvHeap = new D12DescriptorHeap(device, D12HeapType::SRVs, 128);
	rtvHeap = new D12DescriptorHeap(device, D12HeapType::RTVs, 128);
	samplersHeap = new D12DescriptorHeap(device, D12HeapType::Samplers, 128);
	srvCpuHeap = new D12DescriptorHeap(device, D12HeapType::SRVsCPU, 128);
	samplersCpuHeap = new D12DescriptorHeap(device, D12HeapType::SamplersCPU, 128);
...
void D12GraphicsModule::InitializePipeline(D12ShaderPipeline* pipeline)
{
	...
	// Crete signature
	{
		...
		for (auto& parameter : parameters)
		{
			...
			{
				auto& rootParamter = pipeline->FindRootParameter(D12RootParamterType::TableSRV);
			...
			{
				auto& rootParamter = pipeline->FindRootParameter(D12RootParamterType::TableSamplers);
