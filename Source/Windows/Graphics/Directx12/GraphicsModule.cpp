...
//SERIALIZE_METHOD_CREATEGEN_ARG1(GraphicsModule, IShaderArguments, ShaderArguments, const IShaderPipeline*);

DECLARE_COMMAND_CODE(CreateIBuffer);
const IBuffer* GraphicsModule::RecCreateIBuffer(const ExecutionContext& context, size_t size, const IBuffer* gfxBuffer)
{
	auto buffer = GetRecordingBuffer(context);
	auto& stream = buffer->stream;
...
//SERIALIZE_METHOD_CREATEGEN_ARG1(GraphicsModule, IBuffer, Buffer, size_t);
...
bool GraphicsModule::Initialize()
{
	...
	bufferUploadHeap = new BufferHeap(device, 1<<20, 256);
